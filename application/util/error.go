...
func CreateErrorResponse(w http.ResponseWriter, err error, userId string) {
	...
	case define.NOT_FOUND_USER:
		log.Printf(logger.ErrorLogEntry(fmt.Sprintf("userが見つかりませんでした。 userId=%s", userId)))
	...
	case define.FAILED_CREATE_USER:
		log.Printf(logger.ErrorLogEntry(fmt.Sprintf("userの作成に失敗しました。 userId=%s", userId)))
		w.Header().Set("Content-Type", "application/json")
	...
	case define.FAILED_UPDATE_USER:
		log.Printf(logger.ErrorLogEntry(fmt.Sprintf("userの更新に失敗しました。 userId=%s", userId)))
		w.Header().Set("Content-Type", "application/json")
		w.WriteHeader(http.StatusInternalServerError)
	...
	case define.FAILED_DELETE_USER:
		log.Printf(logger.ErrorLogEntry(fmt.Sprintf("userの削除に失敗しました。 userId=%s", userId)))
		w.Header().Set("Content-Type", "application/json")
		w.WriteHeader(http.StatusInternalServerError)
